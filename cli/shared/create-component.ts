...
import genCoreTemplete from '../template/core'
import genTypesTemplete from '../template/types'
...
export default function createComponent(meta: ComponentMeta) {
  ...
  writeFileSync(coreFilePath, genCoreTemplete(name), WRITE_FILE_OPTIONS)

  // 核心文件： 组件类型文件
  const typesFilePath = resolve(compSrcDir, name + '-type.ts')
  writeFileSync(typesFilePath, genTypesTemplete(name), WRITE_FILE_OPTIONS)
